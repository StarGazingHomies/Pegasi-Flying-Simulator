...
OBB::OBB() {
	...
	momentOfInertia = glm::mat3(
		glm::vec3(1.0f / 3.0f * (half_size.y * half_size.y + half_size.z * half_size.z) * mass, 0.0f, 0.0f),
		glm::vec3(0.0f, 1.0f / 3.0f * (half_size.x * half_size.x + half_size.z * half_size.z) * mass, 0.0f),
		glm::vec3(0.0f, 0.0f, 1.0f / 3.0f * (half_size.x * half_size.x + half_size.y * half_size.y) * mass));
...
std::string OBB::toString() {
	std::string ret = "OBB Object: \n";
	ret += "Position: " + glm::to_string(position) + "\n";
	ret += "Rotation: " + glm::to_string(glm::eulerAngles(rotation)) + "\n";
	ret += "Velocity: " + glm::to_string(velocity) + "\n";
	ret += "Acceleration: " + glm::to_string(acceleration) + "\n";
	ret += "Rotational Velocity: " + glm::to_string(glm::eulerAngles(rotVelocity)) + "\n";
	ret += "Rotational Acceleration: " + glm::to_string(glm::eulerAngles(rotAcceleration)) + "\n";
	return ret;
}
//
//OBB2::OBB2() {
...
//	return glm::mat4(glm::angleAxis(angle, axis));
//}
//
...
//}
//
//void PhysicsObject::tick(double time) {
//	// Linear
//	position += velocity * (float)time + 0.5f * acceleration * (float)time * (float)time;
//	velocity += acceleration;
//
//	// Rotational
//	// TODO: Actually handle angle-axis rotation, because rotation velocity can be >360 degrees
//	// Don't even know if this is correct, fix it later...
//	rotation += rotVelocity * (float)time + 0.5f * rotAcceleration * (float)time * (float)time;
//	rotVelocity += rotAcceleration;
//}
//
//void PhysicsObject::debugDraw(glm::mat4 projMatrix, glm::mat4 viewMatrix) {
//	// Gets location of object on screen
//	glm::vec3 pos = glm::vec3(projMatrix * viewMatrix * glm::vec4(position, 1.0f));
//	// Vector in velocity direction
//	glm::vec3 vel = glm::vec3(projMatrix * viewMatrix * glm::vec4(position + velocity, 1.0f));
//	// Vector in acceleration direction
//	glm::vec3 acc = glm::vec3(projMatrix * viewMatrix * glm::vec4(position + acceleration, 1.0f));
//
//	// Draw calls
//	GraphicsPrimitives::drawLine(pos, vel, glm::vec3(1.0f, 0.0f, 0.0f), projMatrix, viewMatrix);
//	GraphicsPrimitives::drawLine(pos, acc, glm::vec3(0.0f, 1.0f, 0.0f), projMatrix, viewMatrix);
//}
//
//std::string PhysicsObject::toString() {
//	std::string repr = "PhysicsObject: \n";
//	repr += "Position: " + glm::to_string(position) + "\n";
//	repr += "Rotation: " + rotToString(rotation) + "\n";
//	repr += "Velocity: " + glm::to_string(velocity) + "\n";
//	repr += "Acceleration: " + glm::to_string(acceleration) + "\n";
//	repr += "Rotational Velocity: " + rotToString(rotVelocity) + "\n";
//	repr += "Rotational Acceleration: " + rotToString(rotAcceleration) + "\n";
//	return repr;
//}
