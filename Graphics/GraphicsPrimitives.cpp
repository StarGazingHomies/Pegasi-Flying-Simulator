...
std::unique_ptr<VAO> GraphicsPrimitives::vao;
std::unique_ptr<VBO> GraphicsPrimitives::vbo;
std::unique_ptr<EBO> GraphicsPrimitives::ebo;

GraphicsPrimitives::GraphicsPrimitives() {}

void GraphicsPrimitives::initialize() {
	vao = std::make_unique<VAO>();
	vbo = std::make_unique<VBO>(std::vector<float>{});
	std::vector<int> vi{};
	ebo = std::make_unique<EBO>(vi);
}
